...
PROVIDER_METADATA = yaml.safe_load(
    Path("tests/integration/auth_provider_tester/metadata.yaml").read_text()
)
REQUIRER_METADATA = yaml.safe_load(
    Path("tests/integration/auth_requirer_tester/metadata.yaml").read_text()
)
PROVIDER_CHARM_NAME = PROVIDER_METADATA["name"]
REQUIRER_CHARM_NAME = REQUIRER_METADATA["name"]
DEPLOY_TIMEOUT = 1000
PROVIDER_APPLICATION_NAME = "auth-provider"
REQUIRER_APPLICATION_NAME = "auth-requirer"
...
class TestAuthLib:
    ...
    async def _deploy_requirer(self, ops_test):
        requirer_charm = self._find_charm(
